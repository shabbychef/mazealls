...
boustro <- function(unit_len,height,width,thickness=8L,angle=90,clockwise=TRUE,start_hole=TRUE) {
	if (start_hole) { 
		bholes <- c(1,3)
		blines <- 1:4
	} else {
	...
	parallelogram_maze(unit_len=unit_len,
										 ...
										 end_side=3)
	if (!last_one) {
		boustro(unit_len,height=height,width=width-thickness,thickness=thickness,angle=180-angle,clockwise=!clockwise,start_hole=FALSE)
	}
}
...
turtle_do({
	turtle_setpos(300,50)
	turtle_setangle(0)
	boustro(unit_len=22,90,70,thickness=12,angle=80)
})
```
